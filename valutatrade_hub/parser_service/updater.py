...
class RatesUpdater:
    ...
    def update_rates(self, source: Optional[str] = None) -> Dict[str, Dict]:
        logger.info(f"Начало обновления курсов" + (f" из источника: {source}" if source else ""))
    ...
    def get_update_summary(self, results: Dict) -> str:
        ...
        if results["status"] == "success":
            summary_lines.append(f"Обновление успешно завершено")
        else:
            summary_lines.append(f"Обновление завершено с ошибками")
